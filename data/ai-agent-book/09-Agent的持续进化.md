...
【图：图9-1 Agent 持续进化的总体闭环】
图中文字：持续进化：从运行经验到新版本 Agent；执行任务；用户、环境与工具交互；记录轨迹；动作、结果与反馈；形成信号；结果·过程·质量；生成候选更新；知识；指令；程序；参数；验证与发布；回归、灰度与回滚；新的 Agent 进入下一轮运行；只有改变后续行为且通过验证的更新，才构成持续进化
...
【图：图9-2 从环境结果到 LLM Rubric 的三层轨迹验证】
图中文字：三层轨迹验证：尽量让证据来自真实环境；结果验证器；测试、金额、订单最终状态；过程验证器；政策、权限、动作与承诺；LLM Rubric；自然、简洁、合规变通；结构化轨迹评价；任务结果；成功、部分成功或失败；维度评价；逐项评分，而不是只给总分；证据位置；标明对话轮次与工具状态；失败标签；保留问题类型，支持后续诊断；证据不足时，允许拒绝评分；可学习的诊断信号；保留成功与失败的位置；说明证据与问题类型；交给独立模块选择更新对象
...
【图：图9-3 持续进化的四种更新方式】
图中文字：能力的表示性质决定主要更新位置；知识；事实、经验与例外；Markdown / 知识库；指令；可语言化的判断原则；Prompt / Skill；程序；确定性流程与强约束；工具 / 工作流 / Harness；参数；高维感知与隐式策略；SFT / 蒸馏 / RL；易更新、可追溯；内化、低推理开销；不是单向阶梯，而是四种互补载体；首要判断：能力能否被事实、语言规则或程序完整表达？；稳定性、成本与更新频率决定具体部署方式，而非能力能否参数化
...
【图：图9-4 从已评价轨迹到经验知识文档】
图中文字：经验知识化：原始轨迹是证据，知识文档才是复用单元；原始轨迹；动作、结果、环境；不可变证据；单次评价；成功、失败、标签；保留证据位置；跨轨迹比较；聚合同类任务；对照成功与失败；提取共性与边界；Markdown 文档；适用场景与策略；例外、禁区与来源；版本与验证时间；检索应用；新任务按需加载；继续产生反馈；新证据用于修订、限定或淘汰旧知识；与第三章共用抽取和检索基础设施；本章新增轨迹评价与行为改进闭环
...
【图：图9-5 在线执行与离线进化的双循环】
图中文字：在线执行与离线进化：正式版本不在运行中直接改写；在线执行循环；稳定版 Agent；处理真实任务；记录轨迹与反馈；离线进化循环；聚合与诊断；寻找共同根因；候选修改；知识·指令·程序·参数；回归与安全验证；保留旧能力；灰度、监控与回滚；晋升为新稳定版；版本化经验日志；通过验证的新版本；可信根：权限、审计、验证与回滚机制不由普通进化流程自行修改
