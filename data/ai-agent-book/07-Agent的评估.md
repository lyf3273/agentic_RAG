...
【图：图7-1 Agent 评估体系的四个环节】
图中文字：① 成功的定义；Pass@k 衡量能力上限 · Pass^k 衡量业务可靠性；② 任务来源；公开基准；借鉴手法 · 粗筛选型；自建业务集；真实任务分布；生产轨迹回流；失败归因的产出；③ 验证方式；← 按任务的可机械验证程度排列 →；确定性验证器；SWE-bench；检查项清单；τ²-bench；Rubric + LLM 评判；开放式任务；配对比较；Chatbot Arena；④ 结果的运用；统计显著性 → 失败归因 → 回归任务 → 模型选型与 Harness 迭代；回归任务沉淀为新用例；支撑设施；可观测性（提供轨迹） · 内部评估基础设施（消融 / AB / 特性开关） · 仿真环境（通向第八章）
...
>
> 【图：图7-3 τ²-bench 的双控环境与分层验证】
图中文字：用户模拟器（LLM）；known_info：John Smith / 555-123-2002 / 在法国；task_instructions：渐进透露 · 情绪 · 事实锚定；验收口径：仅 excellent 算解决；Agent（待评估）；输入：工单 + 领域政策；不可见：用户设备的真实状态；只能引导用户操作，无法代其执行；多轮对话；渐进式信息透露；共享环境（双控：两侧都能改写状态）；设备侧状态；飞行模式 ON · 数据漫游 OFF · 省流模式 · 剩余流量；用户工具：toggle_airplane_mode / toggle_roaming；check_status_bar / run_speed_test；运营商侧数据库；客户 C1001 · 线路 L1001/L1002/L1003 · 套餐 · 账单；Agent 工具：get_customer_by_phone / get_data_usage；enable_roaming / refuel_data；用户执行 toggle_roaming 后环境已改变，Agent 必须重新调用工具才能获知结果——验证因此覆盖 “是否读到了用户侧的操作结果”；任务结束后：四层检查 + 聚合规则；env_assertions；移动数据可用；测速 ≥ 200 / excellent；actions；toggle_airplane_mode；requestor 必须为 user；communicate_info；必要信息是否告知；本题为 null；nl_assertions；自然语言层判定；reward_basis = ["ENV_ASSERTION"] → 仅采信第一层；其余各层照常记录但不计入奖励
...
【图：图7-2 工具调用型与人机交互型评估环境】
图中文字：工具调用型 (Verifiers)；Agent (LLM)；工具执行环境；run_python("def fib(n): ...")；→ {"result": [1,1,2,3,5,8]}；可执行验证器；assert fib(10) == 55 # PASS；assert fib(0) == 0 # PASS；reward = 1.0 (全部通过)；特点: 无需人工/LLM评判；人机交互型 (τ-bench)；用户模拟器；"我的航班有问题"；工具调用；lookup_booking("BK-98712")；→ {"status":"confirmed","flight":"UA123"}；双重验证；① DB: booking.status == "cancelled"；② 对话: 包含 "退款$150"；包含 "3-5个工作日到账"；reward = 0/1 (双重均通过)；特点: 渐进式信息透露+状态验证
...
【图：图7-4 验证方式的谱系：从确定性验证到模型评判】
图中文字：任务的可机械验证程度：高；低；确定性验证器；终态唯一可查；通过 / 不通过；SWE-bench 跑测试；检查项清单；多项确定性检查；按声明的口径聚合；τ²-bench 四层验证；Rubric + LLM 评判；维度可拆但无法机械判定；分维度打分并说明理由；客服质量、报告撰写；配对比较；连维度都难以写出；只判 A 与 B 孰优；Chatbot Arena；确定性验证；结果完全可复现、可纳入持续集成、成本低；代价：只回答对错，不指出问题出在哪里；模型评判；可拆出诊断维度，覆盖写不成断言的任务；代价：引入评判偏差与波动，成本更高
...
【图：图7-5 LLM-as-a-Judge 流水线】
图中文字：Rubric（评分标准）；事实正确性: Essential；逻辑连贯性: Important；幻觉检测: Veto ⚠；完整性: Important；候选回答；Agent 输出:；"退款已处理，$150将在；3-5工作日内到账。"；参考方案（可选）；标准答案 / 评分要点:；必须包含退款金额；必须包含到账时间；不可承诺具体日期；Judge 模型 (GPT-5 / Gemini 2.5)；多源异构评判防止同源偏见；结构化评估输出；事实正确性；4/4；金额和时间均准确；完整性；3/4；缺少退款方式说明；幻觉检测；PASS；无虚假信息；逻辑连贯性；因果关系清晰；评分聚合策略；加权平均: Σ(权重×维度分)；一票否决: 幻觉=FAIL → 总分=0；多评委: 3个Judge取中位数；边界案例标记: 分歧>2分→人审
...
【图：图7-6 Elo 评分与配对比较排名】
图中文字：Chatbot Arena 匿名对决；Model A（匿名）；"退款已处理,将在3-5个；工作日到账您的账户"；VS；Model B（匿名）；"好的退款会处理的"；用户盲选 → A 更好；Elo 更新公式；预期胜率 E_A = 1/(1+10^((R_B-R_A)/400)) | 更新: R_A' = R_A + K*(S_A-E_A)；实时排行榜（示例）；排名；模型；Elo；胜率 vs #2；1；Claude 4 Opus；1352；—；2；GPT-5；1318；3；Gemini 2.5 Pro；1290；45%；4；DeepSeek R2；1265；42%；5；Qwen 3 Max；1230；39%；6；Llama 4 405B；1198；36%；GRPO: 将配对比较引入 RL 训练；一组候选响应 → 相对优势归一化 → 策略更新（绕过显式奖励模型）
...
【图：图7-7 可观测性技术栈】
图中文字：执行追踪树（单次任务）；Trace：帮用户查北京明天天气 (3.2s, $0.008)；LLM Call：意图识别；Claude 4 Sonnet · 0.4s · 320 tokens；Tool：get_weather(Beijing)；MCP Server · 1.8s · 200ms TTFT；├─ HTTP Request；api.weather.com · 1.6s；└─ Response Parse；JSON → 结构化天气数据；LLM Call：生成回复；Claude 4 Sonnet · 0.8s · 580 tokens；Tool：send_message(user)；0.2s · 包含天气摘要；监控仪表盘；成本追踪；今日：$12.30（1,200 次）；本月：$340（34K 次）；异常：task#892 循环调用 search 14 次，耗 $2.1；性能监控；P50 / P95 / P99 延迟：2.1s / 8.4s / 15.2s；工具成功率：94.3%；质量审计；任务成功率：87% 幻觉触发：2.1%；安全违规：0（本月） 用户满意度：4.3/5；闭环：追踪数据 → 发现问题 → A/B 测试 → 提示词版本管理 → 持续优化
...
【图：图7-8 Benchmark 到改进闭环】
图中文字：① 观察：诊断报告；总体成功率: 88% (102/116)；transcription: 0% complex_ui: 17%；math_counting: 0% Wi-Fi操作: 0%；Task 82,102-115 集中失败；② 假设：三层改进框架；表层；H1 设置导航提示 H2 UI规则；中层；H3 修复多模态管道 H4 思考；深层；H5 GPT-5 H6 UI元素树；③ 实验：分阶段验证（每配置 5次×116任务）；H1 导航；设置 0%→75%；token+8%；H3 多模态；转录 0%→80%；延迟+1s；H4 思考；计数 0%→70%；延迟 3x!；H6 元素树；UI 17%→52%；token+30%；④ 决策：成本-收益权衡；✓ H1+H3: 低成本高收益 → 部署；✗ H4: 仅8%任务受益但3x延迟 → 拒绝；✓ H6: 35%提升/30%成本 → 部署；✗ H5: 15s/步不可接受 → 备选；⑤ 迭代：新一轮循环；部署 H1+H3+H6 → 88%→94%；新报告显示不同失败模式:；H7: 条件化思考启用；H8: 扩展手势动作空间；↑ 循环；方法论: 观察→假设→实验→决策→迭代 = 从炼金术到科学工程
...
【图：图7-9 仿真保真度谱】
图中文字：仿真保真度 →；可；扩；展；性；↑；Mock API；单元测试级；百万次/小时；AWorld；MCP 沙盒；126个工具函数；525s/轮 分布式；AndroidWorld；模拟器；116真实应用任务；UI Automator；Isaac Gym；GPU 并行；千级并行实例；精度略妥协；RoboTwin2；物理引擎；高精度碰撞；单实例 CPU；真实世界；完美保真度；不可重置
...
>
>
> 【图：图7-10 OpenVLA 与 RoboTwin2 具身智能环境】
图中文字：多模态观测；头部相机；224×224 RGB；左腕相机；右腕相机；14维关节状态向量；视觉-语言-动作模型 (VLA)；视觉编码器；SigLIP → 视觉 tokens；语言模型；Llama 2 7B backbone；动作解码器；→ 14维连续控制向量；动作分块: 1次生成25个连续动作；指令: "将罐子放入锅中"；SAPIEN 物理引擎；双臂机器人；各7自由度 = 14维动作；环境随机化；位置60cm/朝向±22.5°；碰撞检测 + 物理仿真；刚体/软体/摩擦力；动作；观测；评估指标；成功率；罐子在锅内；且未掉落；完成时间；25步×25动作；= 625控制步；泛化能力；跨位置/朝向；/外观变体；Sim-to-Real；域随机化；→ 真实迁移
>
>
