...
【图：图2-1 上下文窗口的构成概览】
图中文字：系统提示词（System Prompt）；"You are a helpful assistant. You MUST answer concisely."；"Use tools when the user asks for real-time information."；工具定义（Tool Definitions）；{"name": "web_search", "description": "Search the web",；"parameters": {"query": {"type": "string"}}}；对话历史（Conversation History）；user: "北京今天天气怎么样？"；assistant: [tool_call] → get_weather("北京")；tool: {"temp": "23°C", "conditions": "晴"}；本轮思考（Reasoning）；<think>用户问天气，我已经获得了工具结果，；可以直接汇总回答，无需再调用工具。</think>；当前生成位置 →；assistant: "北京今天晴，气温 23°C..." ← LLM 正在生成；上下文；窗口；窗口大小：Qwen3 = 32K tokens | Claude = 200K | Gemini = 2M；所有内容序列化为 token 流 → Transformer 注意力机制处理
...
【图：图2-2 单轮 API 调用的请求与响应结构】
图中文字：Request（Agent 框架构造）；system；开发者写的规则；user；"Hello, who are you?"；调用；Response（API 返回）；assistant；模型生成的回复；"Hi! I'm a coding assistant…"；每次调用都是无状态的——模型需要的所有信息，必须在请求的 messages 列表中完整提供
...
【图：图2-3 两次模型 API 调用的完整交互序列】
图中文字：第 1 次模型 API 调用；messages: system + user；tools: get_current_time,；get_weather；API；assistant：tool_calls；get_current_time(timezone="America/Vancouver")；get_weather(city="Vancouver", unit="celsius")；两者无数据依赖 → 可并行；Agent 框架并行执行两个工具；第 2 次模型 API 调用；messages: + tool 结果；温哥华时间 & 天气；追加到消息历史，再次请求模型；assistant：最终回复；无工具调用 → 结束循环；"现在是…，天气…"；无状态 API 下，每一轮都要把完整的消息历史重新发送给模型
...
【图：图2-4 Agent 每次调用模型时的上下文构成】
图中文字：静态前缀（每一轮都不变）；System Prompt（系统提示词）；Tool Definitions（工具定义）；对话历史 / 轨迹（随交互不断增长 →）；user；assistant；tool 结果；…；“静态前缀 + 轨迹”结构：前缀不能动（利于 KV Cache），轨迹可压缩
...
>
>
> 【图：图2-5 本地 LLM 工具调用架构】
图中文字：用户请求；“帮我联系 Xfinity 砍价”；本地 LLM 服务；vLLM/Ollama（OpenAI 兼容）；模型推理；决定并生成 tool_call；本地工具执行；调用函数 / 外部 API；工具结果回传模型，生成最终回复
>
...
>
> 【图：图2-6 注意力机制的直观理解】
图中文字：① 「怎么样」对前文每个词的注意力权重；北京；Key · 权重 0.35；的；Key · 权重 0.05；天气；Key · 权重 0.55；怎么样；Query（当前）；Query 与各 Key 打分 → 归一化为权重 → 对 Value 加权求和（重点参考「天气」）；② 注意力热力图：每个词只能看自己与前文（因果三角）；Key →；Query ↓；1.00；0.30；0.70；0.20；0.10；0.35；0.05；0.55；格子越深 = 注意力越集中；上三角为空 = 不能看到还没生成的词
>
>
...
【图：图2-8 Chat Template 的 Token 结构】
图中文字：结构化 API 消息；system；“You are a helpful assistant.”；user；“北京今天天气怎么样？”；assistant；（待生成）；Chat Template；模型实际处理的线性 Token 流；<|im_start|>system；You are a helpful assistant.<|im_end|>；<|im_start|>user；北京今天天气怎么样？<|im_end|>；<|im_start|>assistant；特殊标记界定角色与消息边界，拼成一段连续序列
...
【图：图2-9 API 消息到模型 Token 流的转换】
图中文字：API 层面（开发者看到的）；{；"role"；:；"system"；,；"content"；"你是助手"；}；"user"；"你好"；模型层面（经 Chat Template 转换后）；<|im_start|>；system；你是助手；<|im_end|>；user；你好；assistant；（模型从这里开始生成）
...
【图：图2-10 Prompt Cache：跨请求复用前缀的 KV Cache】
图中文字：请求 1；System Prompt + Tools (1200 token)；user: "天气如何？"；→ 生成回答；请求 2；System Prompt + Tools（缓存命中 ✓）；user: "时间几点？"；Prompt Cache 命中：前缀 KV 复用；系统提示变了；Prompt Cache 失效：前缀 KV 重算；请求 3；"Time: 10:30:45" + System Prompt + Tools（缓存失效 ✗）；变动点在最前 → 其后全部重算；性能对比（3000 token 总上下文）；缓存命中；缓存失效；TTFT *；~0.5 秒；3 - 5 秒；成本；仅新 token 计费；变动点后的 token 全部重新计费；* TTFT 为示意值，随模型、硬件与部署方式而变，此处仅用于说明相对量级差异
...
【图：图2-11 Skills 渐进式披露机制】
图中文字：第一层：元数据（启动时加载，~300 tokens）；skills: [{name: "PPTX", desc: "Create PowerPoint presentations from content"}；{name: "PDF", desc: "Extract and analyze PDF documents"}, ...]；任务触发："从论文生成 PPT"；第二层：SKILL.md 核心流程（按需加载，~2K tokens）；PPTX Skill 核心流程:；1. markitdown 提取文本 → 2. 解压 PPTX 访问 XML；3. 修改 slide{N}.xml 内容 → 4. 重新打包为 .pptx；引用: → html2pptx.md | → reference.md | → scripts/；需要详细方法："用 HTML 模板创建 PPT"；第三层：子文档（选择性深入，按需加载）；html2pptx.md；HTML 模板 → PPT；的完整工作流；reference.md；XML 格式规范；和技术细节；scripts/*.py；可执行工具:；thumbnail.py 等；元数据固定 → KV Cache 友好 | 动态内容追加 → 不破坏缓存
...
【图：图2-12 启用 Skills 后 Agent Trajectory 的完整结构】
图中文字：messages: [；{ role: "system", content: "你是 Claude Code 助手..." }；tools: [Skill, Read, Bash, Edit, Write, ...]；固定不变；(KV Cache 缓存)；{ role: "user", content: "帮我从这个 PDF 生成 PPT" }；{ role: "user", isMeta: true,；content: "<system-reminder>；Available skills: pdf, pptx, ...</system-reminder>" }；ⓐ Skill listing；Harness emit-once；~300 tokens；{ role: "assistant", tool_calls: [Skill(skill: "pptx")] }；{ role: "tool", content: "Launching skill: pptx" } ← 占位符；content: "Base directory: ...\n# PPTX Skill；## Workflow: 1. Use markitdown..." }；ⓑ Skill content；Skill 工具 emit-once；~2k tokens；{ role: "assistant", tool_calls: [Read(file: "input.pdf")] }；{ role: "tool", content: "...PDF 文本内容..." }；{ role: "assistant", tool_calls: [Write(file: "slides.html")] }；{ role: "tool", content: "Wrote 12345 bytes" }；后续 tool_use /；tool_result 持续；append 到末尾；... 后续轮次 ...；]；ⓐ 与 ⓑ 都是 emit-once：付一次 cache_creation 后永久驻留缓存前缀，不会随后续 tool_use 移动

【图：图2-13 KV Cache 随 Agent Trajectory 增长的演化】
图中文字：Turn 1 完成时；Turn 2 完成时；Turn 3 完成时；(首次加载 PPTX skill)；(读取 PDF 文件)；(写出 HTML)；system；NEW；tools；user_q1；★ skill_listing；asst: Skill(pptx)；tool_result；★ skill_content；本轮 cache_creation；≈ 2.5k tokens；HIT；asst: Read(pdf)；≈ 0.5k tokens；asst: Write(html)；≈ 0.4k tokens；NEW = 该轮新增 token，付一次 cache_creation；HIT = 已在缓存前缀，本轮免费命中；— 该轮尚未生成的内容；★ 标记 emit-once 的 attachment：仅 Turn 1 付一次 cache_creation，；之后所有轮次永久 HIT，边际成本为零。；注意：每条消息的索引位置一旦插入就不再移动；新内容只追加到 array 末尾。
...
【图：图2-14 Agent 状态栏架构】
图中文字：无状态栏；有状态栏；system:；System Prompt + Tools；user:；"帮我联系 Xfinity 砍价"；assistant:；phone_call(Xfinity) → 第 1 次；tool:；结果: 等待 45 分钟, 未接通；web_search("Xfinity deals")；结果: [大量搜索内容...]；phone_call(Xfinity) → 第 2 次；结果: 接通，报价 $65/月；phone_call(Xfinity) → 第 3 次；结果: 确认降价到 $59/月；"能不能再打一次催一下？"；→ 模型需扫描全部上下文来"数"；拨打了几次电话，容易数错；...:；[ 同样的轨迹内容 ]；<agent_status>；phone_call 已调用 3 次 (Xfinity: 3)；约束检查: 已达上限 (3/3) ✗；TODO: [✓]联系Xfinity [✓]确认降价；当前时间: 2025-09-14 10:30；当前状态: 等待用户确认；</agent_status>；→ 模型直接读取已提炼状态；准确遵守约束，不再拨打；VS
...
【图：图2-15 Agent 状态栏在 API 消息列表中的插入位置】
图中文字：messages: [；{ role: "system", content: "你是电信客服..." }；tools: [cancel_plan, query_records, ...]；固定不变；(KV Cache 缓存)；{ role: "user", content: "帮我取消套餐" }；{ role: "assistant", tool_calls: [cancel_plan(...)] }；{ role: "tool", content: "该套餐有合约期..." }；{ role: "assistant", content: "您的套餐在合约期内..." }；... 更多对话轮次 ...；{ role: "user", content: "那帮我查一下通话记录" }；用户追问；{ role: "user", content: "<agent_status>；已呼叫 3/3 次 · TODO: 取消套餐(进行中)</agent_status>" }；Agent 框架插入；的 Agent 状态栏；]；模型从此处开始生成；← 紧邻模型生成位置，获得最高注意力权重
...
【图：图2-16 上下文压缩策略对比】
图中文字：策略；Token 用量；压缩率；迭代次数；结果；可视化（Token 用量对比）；无压缩；166,043；102.1%；5；✗ 失败；个体摘要；276,608；10.9%；12；✓ 成功；组合摘要；93,449；4.3%；10；上下文感知；40,157；3.0%；7；感知+引用；222,992；4.1%；自适应窗口；174,601；102.4%；上下文感知压缩：相比无压缩节省 76% token，并列最少迭代次数；关键：将查询意图和已有信息纳入压缩决策
...
>
> 【图：图2-17 六种压缩策略的处理流程】
图中文字：每次搜索平均返回 ~52K 字符 → 各策略以不同方式处理；① 无压缩；直接保留；完整原文放入上下文；166K tok · 102.1% · 失败；② 个体摘要；独立摘要；每个结果独立生成 2-3 段摘要；277K tok · 10.9% · 12轮；③ 组合摘要；合并摘要；所有结果拼接后统一摘要；93K tok · 4.3% · 10轮；④ 上下文感知；智能压缩；Given query + context → 针对性压缩；40K tok · 3.0% · 7轮；⑤ 感知+引用；智能+溯源；压缩内容 + 保留 URL 引用标记；223K tok · 4.1% · 10轮；⑥ 自适应窗口；延迟压缩；< 80% 窗口保留原文，超出批量压缩；175K tok · 102.4% · 7轮
>
