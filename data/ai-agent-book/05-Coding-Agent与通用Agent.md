...
读者可能会问，为什么是七个工具，不是六个工具？事实上，一个 Bash Shell 工具就能覆盖大部分操作。OpenAI Codex 的工具集就极为精简：它以 shell 作为唯一的通用执行入口，用命令完成目录浏览、文件查找与读取；文件修改则交给一个语法受约束的 `apply_patch` 工具——把编辑表达为结构化补丁而非自由文本命令，从而降低改错文件的概率。但其他一些 Agent 仍然保留单独的文件读写工具。本书的七个工具是为了方便读者理解 Coding Agent 所需的基本能力。
...
【图：图5-1 OpenClaw 架构中的 Coding Agent 核心】
图中文字：多平台消息网关（用户交互层）；WhatsApp；Telegram；iMessage；Slack；CLI；自然语言请求；Coding Agent 运行时（推理 + 执行核心）；Code Interpreter；代码执行；Bash Shell；系统命令；Read File；读取文件；Write File；写入文件；Edit File；编辑文件；Glob；文件搜索；Grep；内容搜索；网络搜索模块；Deep Research；Web 请求 · 解析；浏览器自动化；Computer Use；Playwright DOM；读 / 写文件；文件系统（记忆 · 知识 · 能力中枢）；MEMORY.md；高层级事实 / 用户偏好；daily/YYYY-MM-DD.md；按日归档 / 交互日志；SOUL.md；Agent 身份与行为规则；知识库文件；任务经验 / 自我进化；Git 版本控制；记忆回滚 / 历史审计；大模型 = 新操作系统：屏蔽智能复杂性，提供统一抽象
...
【图：图5-2 Coding Agent 工作流程】
图中文字：① 项目文档化；read_file；README.md,；ARCHITECTURE.md；glob；**/*.py, **/*.ts；write_file；→ 生成 CLAUDE.md 项目指南；② 需求理解；ask_user；"优化目标是延迟还是吞吐？"；grep；"latency|throughput" src/；src/config.py (当前参数)；③ 设计文档；design.md (方案对比)；提交设计 → 等待审批；—；人类审查后 → 继续；④ 编码与测试；edit_file；old_str→new_str 修改代码；bash；pytest tests/ -v；修复失败测试 → 重跑；⑤ 审查与交付；ruff check src/ (lint)；自审: 可读性/安全/性能；更新 ARCHITECTURE.md；闭环反馈机制；测试失败 → 修改代码 → 重新测试；④内循环: 平均 2-3 轮收敛；Lint 错误 → 即时修复 → 再检查；⑤内循环: 编辑后自动触发；审查发现问题 → 回到④修改；⑤→④回退: 保证交付质量；Agent 状态栏: cwd, git branch；Agent 状态栏: 未暂存变更；工具输出: 头尾截断；持久化终端会话；计划先于行动 · 验证贯穿始终 · 文档与代码共同演化
...
【图：图5-3 Coding Agent 搜索工具对比】
图中文字：正则内容匹配 (grep)；查询:；rg "def handle_.*" --type py；结果:；src/api.py:42: def handle_request(..)；src/api.py:89: def handle_timeout(..)；src/ws.py:15: def handle_connect(..)；精确文本 → 所有出现位置；文件名匹配 (glob)；glob: **/test_*.py；tests/test_api.py；tests/test_auth.py；tests/unit/test_parser.py；路径模式 → 不读取文件内容；语义代码搜索；"处理用户输入验证"；[0.91] src/validators.py:validate_input()；[0.87] src/forms.py:sanitize_fields()；[0.82] src/api.py:check_params()；自然语言 → 向量+BM25混合；符号定义/引用；find_references: UserService；定义: src/services/user.py:12；引用: src/api/routes.py:34 (import)；引用: src/api/routes.py:56 (调用)；引用: tests/test_user.py:8 (测试)；AST级 → 消除同名歧义
...
【图：图5-4 五种文件编辑方案对比】
图中文字：Diff + Apply Model；LLM 输出 diff 描述:；- def foo(x):；- return x；+ def foo(x, y=0):；+ return x + y；→ 小模型定位并应用；优势: 关注点分离；劣势: 微小偏差致错位；Old String → New String；old: "def foo(x):\n；return x"；new: "def foo(x, y=0):\n；return x + y"；→ 字符串精确匹配替换；优势: 可预测、无歧义；劣势: 大段删除需全输出；行号定位；删除行 42-43，插入:；def foo(x, y=0):；return x + y；→ 行号精确指定范围；优势: 大段操作高效；劣势: 长文件行号易错；类 Vim 命令；42G (跳转42行)；cw (替换单词)；dd (删除行)；yy/p (复制/粘贴)；→ 丰富的编辑语义；优势: 移动/重组高效；劣势: 弱模型出错多；首尾匹配；start: "def foo(x):"；end: " return x"；new: "def foo(x, y=0):；→ 只需边界即可定位；优势: 大段删除免全输出；劣势: 需边界组合唯一；实际采用情况；Old→New；Claude Code；IDE 深度集成场景；Diff + Apply；Cursor；部分定制方案；Vim 命令；实验性方案
...
【图：图5-5 PPT 生成的提议者-审核者机制】
图中文字：Proposer Agent；输入: 论文/资料内容；paper.pdf → 提取章节/论点/图表；输出: Slidev Markdown；---；layout: two-cols；# Transformer 架构；::left::；- 自注意力机制；- 多头注意力；::right::；<img src="fig3.png" />；Reviewer Agent；步骤1: 渲染截图；slidev export --per-slide；→ slide-01.png, slide-02.png ...；步骤2: Vision LLM 审查；审查维度:；✓ 文字是否溢出边界；✓ 布局是否拥挤；✓ 图片尺寸是否合适；✗ Slide 3: 内容过多，建议拆分；✗ Slide 7: 字体过小，增大到 14pt；Slidev 代码；修改建议；迭代 2-3 轮；为什么分离 Proposer 和 Reviewer？；单 Agent 问题；数十页渲染截图 → 上下文膨胀；代码 + 截图混合 → 注意力分散；分离的优势；Reviewer 独立上下文 → 只看截图+代码；Proposer 专注代码 → 只收修改建议；实际效果；显著减少上下文占用；修复准确率提升显著
...
>
>
> 【图：图5-6 论文到讲解视频的端到端流水线】
图中文字：阶段一：PPT 生成 (Proposer-Reviewer)；PDF 输入；paper.pdf；解析章节结构；提取图表引用；内容规划；10-20 页结构；核心论点提取；图表分配到页；Slidev 生成；逐页生成代码；layout: two-cols；代码+图片混排；渲染检查；export --per-slide；Vision LLM 审查；溢出/拥挤检测；迭代修复；Reviewer→Proposer；修改 Slidev 代码；重新渲染验证；PPT 完成；阶段二：视频合成；逐页截图；slide-01.png；slide-02.png；...；讲解稿生成；LLM 生成口语化；讲解文字(每页)；引导性叙述；TTS 合成；文字 → 语音；speech-01.mp3；speech-02.mp3；音画同步；ffmpeg 合成；截图时长=音频时长；转场效果；最终视频；output.mp4；5-15 分钟；视觉+听觉双通道；验收标准；PPT；10-20 页 · 覆盖主要贡献 · ≥3 原图表；渲染；零文字溢出 · 布局合理 · 图文匹配；视频；5-15 分钟 · 音画同步 · 叙述连贯
>
>
...
>
> 【图：图5-7 生产日志智能诊断流水线】
图中文字：① 日志采集；trajectory_001.json:；{"role":"user","content":；"取消订单 #12345"}；{"role":"assistant",；"tool_call":"cancel_order"}；{"role":"tool","result":；"ERROR: no insurance"}；→ Agent 未告知用户原因；② LLM 分析；输入: 轨迹 + 架构文档 + PRD；分析维度:；- 执行流程是否符合预期；- 工具调用是否正确；- 错误处理是否得当；- 用户体验是否满意；→ 定位偏差环节和模块；③ 结构化报告；问题报告:；优先级: P1 (用户流失风险)；模块: cancellation_handler；描述: 取消失败后未向；用户解释原因和替代方案；建议: 添加失败原因说明；和购买保险的引导；④ 回归测试用例生成；def test_cancel_no_insurance():；"""轨迹 #001, 轮次 3-5"""；# 重放: 用户请求取消经济舱；resp = agent.run(；"取消订单 #12345")；# 验证: 应解释原因；assert "保险" in resp.text；assert "替代方案" in resp.text；# 验证: 不应直接报错；assert "ERROR" not in resp.text；⑤ GitHub Issue 自动创建；gh issue create \；--title "P1: 取消失败缺少；用户引导" \；--body "**问题**: Agent 在；cancel_order 失败后直接；返回错误，未解释原因...；**轨迹**: #001 轮次 3-5；**测试**: test_cancel_..." \；--assignee @backend-team；端到端自动化：日志 → 分析 → 报告 → 测试 → Issue；通过 MCP 对接 GitHub · 测试框架自动重放验证；将人工诊断成本从小时级降低到分钟级
>
>
...
【图：图5-8 动态表单生成流程】
图中文字：用户输入；"我想订去北京的机票"；LLM 分析 → 生成表单代码；<form id="clarify">；<input type="text"；name="from" label="出发城市"/>；<input type="date"；name="depart" label="出发日"/>；<select name="type">；<option>单程</option>；<option>往返</option>；</select>；</form>；渲染后的表单界面；出发城市；上海；出发日期；2025-08-15；旅行类型；往返 ▾；返程日期；2025-08-22；提交；结构化 JSON 返回；{"from": "上海",；"depart": "2025-08-15",；"type": "往返",；"return": "2025-08-22"}；Agent 携带完整参数继续执行任务；search_flights(from='上海', to='北京', depart='2025-08-15', ...)；对比: 纯文本 vs 表单；文本问答: 10 轮对话；Q1: 从哪出发? A: 上海；Q2: 几号? A: 8月15；Q3: 单程还是往返? ...；动态表单: 1 次提交；所有信息一次收集完成；级联逻辑自动处理；表单代码由 LLM 动态生成 → 级联逻辑: 选"往返"时自动显示返程日期
...
【图：图5-9 SQL 查询 Agent 流程】
图中文字：传统模式: 数据经过 LLM（低效）；✗ 低效；用户；"每部门；人数？"；LLM；生成；SQL；DB；执行；查询；阅读；5000行；文字；描述；问题: LLM 抄写数据易出错 · 消耗大量 token · 延迟高；Artifact 模式: 数据直达前端（高效）；✓ 高效；LLM 只生成代码；build_artifact(；type="sql",；code="SELECT dept,；COUNT(*) as cnt；FROM employees；GROUP BY dept")；前端直接执行；┌────────┬──────┐；│ dept │ cnt │；├────────┼──────┤；│ 研发部 │ 42 │；│ 市场部 │ 28 │；└────────┴──────┘；可视化 Artifact；第二个 artifact:；type="chart",；code="bar(data)")；数据流: DB → 前端 → 可视化 （完全绕过 LLM）；LLM 只负责生成代码，不参与数据传递；SQL结果直传
...
【图：图5-10 Agent 自举循环】
图中文字：尘埃→恒星；物理定律；恒星→行星；引力聚合；行星→生命；DNA 自复制；生命→智能体；代码自举；DNA 自复制: 随机变异 + 自然选择；不理解自身 · 不能定向修改 · 37亿年盲目试错；Agent 自举: 理解代码 + 定向设计；理解自身机制 · 有目的地创造 · 继承最佳实践；原始 Agent (自身代码)；系统提示词；你是一个航空客服；取消规则: ...；转接规则: ...；工具: cancel_order；Agent 框架代码；loop:；msg = llm(ctx)；if tool_call:；exec(tool)；工具定义 + MCP 集成 + 消息格式；经过验证的高质量实现；复制 + 修改；新 Agent（定向修改后）；新系统提示词；你是一个电商客服；退款规则: ...；物流查询: ...；工具: refund_order；继承框架代码；新工具 + 新业务逻辑；架构框架完全继承 → 质量有保障
...
>
>
> 【图：图5-11 能创造 Agent 的 Agent 流水线】
图中文字：用户需求；"创建一个电商退款客服 Agent"；Meta-Agent (Coding Agent)；① 阅读参考代码；read_file:；agent.py；tools/*.py；system_prompt.md；config.yaml；→ 理解架构模式；② 复制脚手架；cp -r reference/；→ new_agent/；保留：；Agent 循环框架；消息格式 / KV 优化；③ 定向修改；edit_file:；→ 电商退款规则；tools/refund.py；→ 新增退款工具；④ 验证测试；bash:；python agent.py；→ 启动新 Agent；→ 发送测试消息；→ 检查工具调用；→ 验证对话流程；生成的新 Agent；电商退款规则；退款 / 查询工具；继承框架代码；模型 / 参数配置；从零生成：缺乏最佳实践；上下文管理随意 · 工具设计不规范 · API 过时；基于范例修改：继承最佳实践；标准消息格式 · 规范工具设计 · 现代 API
>
