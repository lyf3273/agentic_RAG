...
【图：图1-1 Agent 与 Environment 的闭环交互，以及 Agent 内部的 Model–Harness 结构】
图中文字：Agent（智能体）；Harness（模型运行与交互层）；上下文；观察 · 历史 · 记忆 · 任务状态；Model；理解 · 推理 · 选择下一步行动；工具与动作接口；循环 · 状态管理 · 权限 · 验证 · 纠正；Environment（环境）；状态与转移规律；当前状态；执行行动后产生新状态；文件系统 · 数据库；网页 · API · 应用程序；用户 · 其他 Agent；仿真或物理世界；观察；行动
...
【图：图1-2 Agent 能力更新的三个层次】
图中文字：Agent 的行为如何发生改变？；任务内适应；Context adaptation；主要载体；当前上下文；示例 · 状态 · 检索结果；更新特性；即时、低成本；任务结束后不自动保留；外部制品更新；External artifacts；知识 · 指令 · 程序；文档 · Prompt/Skill · Harness；跨任务持久、可审计；依赖检索或工具调用；模型参数更新；Parameter update；模型权重；SFT · 偏好训练 · RL；高维能力、广泛泛化；训练与回归成本较高；三者协同：临场适应 · 可控积累 · 能力内化
...
>
> 【图：图1-3 实验 1-1——上下文消融实验设计】
图中文字：系统指令；工具定义；思考过程；历史记录；工具结果；结果；完整基线；✓；✓ 正常工作；无工具定义；✗；✗ 无法调用工具；无思考过程；△ 决策不连贯；无历史记录；△ 重复操作；无工具结果；✗ 盲目循环
>
...
【图：图1-4 Agent 轨迹——多币种汇总任务的 ReAct 循环】
图中文字：第 1 轮；user；"计算年度总收入：Q1 $2.5M, Q2 €2.1M, Q3 £1.8M"；assistant.reasoning；"需要将 EUR 和 GBP 转换为 USD，再汇总计算"；assistant.tool_calls；convert_currency(2100000, "EUR", "USD")；convert_currency(1800000, "GBP", "USD")；tool (结果)；EUR→USD: 2,282,608.70；GBP→USD: 2,278,481.01；第 2 轮；"已获得汇率，调用代码解释器汇总"；code_interpreter("total = 2.5M + 2.28M + 2.28M")；第 3 轮；assistant.content（最终回答）；"年度总收入 $7,061,089.71，季度均值 $2,353,696.57"；轨迹；=；LLM 每次；调用时看到；的完整输入；关键特性；上下文累积；每轮都看到全部历史；结构化轨迹；user / assistant / tool
...
>
> 关键观察包括：模型自己决定何时搜索、搜索什么，展现了真正的自主性；它能根据搜索结果动态调整策略，自主判断信息是否充足。这里需要厘清一个常见的误解，关键在于分清两件事的归属。**强化学习写进参数的是决策**——何时该调用工具、调用哪个、传入什么参数、拿到结果后是否继续、如何把几十上百次调用串联成连贯的推理。**工具本身及其执行则由 Agent 框架（或 API 内置工具）提供**——`web_search`、`code_runner` 的真实实现、代码沙盒环境、调用的发起与结果回传，都在模型之外的基础设施里完成（Kimi 通过名为 Formula 的服务端脚本引擎运行这些官方工具）。因此编排循环并没有消失，只是决策权交给了模型；至于循环本身跑在哪里，取决于接入方式。在实验 1-3 的 Responses API 路径中，搜索、代码执行和多轮编排整体在服务端闭环完成；而在实验 1-2 的 Kimi 路径中，Formula 只负责在服务端执行搜索本身，“调用模型 → 追加工具结果 → 再次调用”的 ReAct 循环仍由客户端代码驱动（见 `chapter1/web-search-agent/agent.py` 中的 `while` 循环）[^ch1-2]。
>
...
>
> 图1-5 展示了“模型即 Agent”范式下原生工具调用的完整架构，以及 Kimi K3 / GPT-5.6 在实际任务中的 ReAct 执行过程。注意图中“客户端无需手写 ReAct 循环”只对 Responses API 路径成立：两条路径都由模型决定何时调用哪个工具、工具都在服务端执行，区别在于 Kimi 路径的循环仍由客户端驱动。
>
> 【图：图1-5 “模型即 Agent” 架构——原生工具调用】
图中文字：LLM（Kimi K3 / GPT-5.6）；RL 训练后的原生 Agent 能力；原生工具；web_search；code_interpreter；更多工具...；RL 训练信号；用户：搜索最近一个月；的比特币走势；ReAct 循环（Responses API 路径：服务端 Harness 闭环执行）；带着观察结果继续下一轮；思考：需要搜索实时；数据，再用代码分析；第 1 轮：调用 web_search；"BTC price last month"；第 2 轮：code_interpreter；结果：[价格数据]；$67,230 → $71,450；最终输出：技术分析；报告 + 可视化图表；与传统框架的区别；✓ 模型自主决定工具调用；✓ 工具在服务端执行；✓ Responses API：循环在服务端；※ Kimi 路径：客户端仍驱动循环
...
早期的 Agent 框架主要关注上下文与工具：给模型工具、给模型上下文，让它“能做事”。而生产级 Agent 系统的重心已经转向约束、验证与纠正：确保工具调用是安全的、上下文是经过管理的、错误是可纠正的。
...
【图：图1-6 自主 Agent 的执行循环】
图中文字：① 思考（Reasoning）；"还需更多信息"；② 行动（Acting）；web_search(...)；③ 观察（Observing）；tool_result: "..."；退出条件（任一满足）；① 任务完成；② 调用 final_answer；③ 无工具调用返回；④ 错误次数超限；⑤ 达到最大轮次；判断依据；满足退出；条件？；否；继续下一轮循环；是；输出最终结果
