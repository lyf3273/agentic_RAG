...
【图：图10-1 共享上下文与不共享上下文对比】
图中文字：共享上下文（继承式协作）；阶段 1：需求分析师；sys: "你的职责是充分理解需求..."；tools: [ask_question, save_req]；user: "写一个 CSV 分析脚本"；agent: "需要处理哪些文件类型？"；阶段 2：软件工程师；sys: "根据已确认需求编写代码..."；tools: [write_file, execute_code]；agent: write_file("analyze.py", ...)；agent: execute_code("python test.py")；阶段 3：代码审查员；sys: "审查代码质量和安全性..."；tools: [run_linter, run_tests]；agent: run_linter → 2 warnings；agent: approve_code()；↑ 所有阶段共享同一对话历史；✓ 完整轨迹；✗ 上下文快速膨胀；不共享上下文（隔离式协作）；Glossary Agent；sys: "识别术语并翻译..."；tools: [search_dict, write_file]；→ glossary.json；Translation Agent；sys: "翻译本章内容..."；tools: [read_file, write_file]；→ chapter1_zh.md；Proofreading Agent；sys: "检查术语一致性..."；→ review_report.md；共享文件系统；glossary.json chapter1_zh.md review_report.md；+ 工具调用参数传递结构化数据；✓ 模块化 · 可扩展 · 并行；✗ 信息同步复杂
...
【图：图10-2 Agent 虚拟文件系统的四类区域挂载结构】
图中文字：Agent A；Agent B；虚拟文件系统 /；统一接口：read_file · write_file · list_dir；用户；上传 / 下载；挂载 mount；Agent 专属工作区；/scratch/<id>；Scratchpad；私有 · 仅该 Agent；随实例销毁；读写 · 无需并发控制；每个 Agent 各一份；多 Agent 共享空间；/workspace/shared；Shared Workspace；用户可见 · 持久化；读写 · 需并发控制；乐观锁 · worktree；外部挂载资源；/mnt/gdrive · /mnt/notion；经适配器 adapter；受外部授权约束；多为只读 · 写需谨慎；延迟高 · 一致性弱；系统内置资源；/skills；Skills · 模板 · 手册；全局共享 · 只读；跨会话稳定；渐进式披露；外部数据源；Google Drive · Notion
...
【图：图10-3 提议者-审核者循环】
图中文字：Proposer Agent；输入: 论文扩展摘要 (2000 字)；---；theme: academic；# Transformer 注意力机制；## 核心思想；- 自注意力计算 Q·K^T/√d；- 多头注意力并行处理；理解内容结构 → 分解为幻灯片页面；Reviewer Agent；① Slidev 渲染 → PDF/PNG；② Vision LLM 多维度评估；结构化反馈:；页码 问题类型 严重度；P3 内容过密 高；P7 字体过小 中；P11 配色不协调 低；渲染 + 视觉分析 → 可执行改进建议；Slidev 代码；结构化反馈；迭代改进过程；Round 1；12 页初稿；5 个问题；Round 2；14 页（拆分密集页）；2 个问题；Round 3；14 页（字体修正）；0 个问题 ✓；为何不用单 Agent？；单 Agent: 渲染图 ×N 轮 → 上下文爆炸；(1080p 截图 = 数千 token × 14 页 × 5 轮)；双 Agent: Reviewer 仅看当前版本；Proposer 仅累积文本反馈 → 上下文干净
...
2025 年的 Plan-and-Act 论文 [^plan-and-act-2025] 对此做了实证分析：在 Planner-Executor 双 Agent 架构中，**弱规划者是整个系统最关键的瓶颈**。当 Planner 的规划质量足够高时，即使 Executor 比较简单也能取得好结果；反之，如果 Planner 的任务分解有误，后续所有 Executor 的工作都建立在错误的前提上。该研究在 WebArena-Lite 基准上取得了 57.58% 的成功率，核心贡献正是改善了 Planner 的规划能力，而非 Executor 的执行能力。这一发现的启示是：应当将最强的模型和最精心设计的提示词分配给 Manager（规划者），而不是将资源平均分配给所有 Agent。
...
【图：图10-4 Manager 顺序协调】
图中文字：Manager Agent；任务理解 → 分解 → 调度 → 综合；工具集: [call_agent_A, call_agent_B,；call_agent_C, search, write_file]；Sub-Agent A；角色: 数据收集；搜索技术文档；提取关键信息；Step 1；Sub-Agent B；角色: 分析处理；对比分析数据；生成统计报告；Step 2；Sub-Agent C；角色: 报告生成；撰写最终报告；格式化输出；Step 3；顺序执行流；Manager 调用 A；→ A 返回数据；→ Manager 传给 B；→ B 返回分析；→ Manager 传给 C；→ C 返回报告；Manager 视角：调用 Agent = 调用工具（发请求 → 获响应）
...
>
>
> 【图：图10-5 书籍翻译 Agent 架构】
图中文字：Manager Agent；任务规划 · 进度监控 · 异常处理 · 结果综合；Glossary Agent；术语对照表；接收全书 → 识别专业术语；搜索专业词典 + 翻译规范；输出: glossary.json；{"attention": "注意力",；"transformer": "Transformer",；"backprop": "反向传播"}；Translation Agent ×N；章节翻译；输入: 章节 + 术语表 + 指南；术语严格按表翻译；输出: chapter{n}_zh.md；"...注意力机制通过计算；Query·Key^T 的相似度..."；Proofreading Agent；全文审校；扫描验证术语一致性；检查流畅性和可读性；输出: review_report.md；P3: "注意力"→"关注"不一致；P8: 长句建议拆分；术语表；译文；共享文件系统；glossary.json；chapter{1..10}_zh.md；章节译文；review_report.md；审校报告；translation_guide.md；翻译指南；上下文隔离优势；Glossary: 仅看术语 | Translation: 仅看当前章节+术语表 | Manager: 仅维护文件索引
>
>

**并行协调形态。**

【图：图10-6 Manager 并行协调】
图中文字：Manager Agent；并行调度 · 实时监控 · 结果汇总；消息总线（Message Bus）；Agent 1；数据采集；运行中 ◎；独立上下文；Agent 2；内容分析；Agent 3；图表生成；已完成 ✓；Agent 4；格式校验；等待中 ○；消息总线通信示例；Manager → Agent 1；{"type":"start","task":"采集 arxiv 论文","params":{"query":"LLM agent"}}；Agent 3 → Manager；{"type":"completed","agent_id":"3","result":"charts/fig1.svg 已生成"}；Agent 1 → Agent 2；{"type":"data_ready","source":"agent_1","file":"raw_data.json"}；Manager → Agent 4；{"type":"start","depends_on":["agent_2","agent_3"]}
...
>
>
> 【图：图10-7 Phone 与 Computer 双 Agent 架构】
图中文字：Phone Agent；Node.js · 实时语音通话；用户语音；麦克风输入；VAD + ASR；Silero VAD → STT 转录；LLM 推理；理解意图 + 提取信息；TTS 合成；生成语音回复 → 播放；Computer Agent；Python · 浏览器自动化；屏幕截图；浏览器当前页面；Vision LLM；理解页面结构 + 表单字段；动作规划；定位字段 → 规划输入序列；执行操作；点击 / 输入 / 提交；WebSocket 双向通信 (ws://localhost:8849)；实时双向消息流（边打电话边用电脑）；Phone → Computer；[FROM_PHONE_AGENT] 用户说姓名是张三；Computer → Phone；[FROM_COMPUTER_AGENT] 已填写姓名，需要证件号；[FROM_PHONE_AGENT] 证件号 310101199001011234；[FROM_COMPUTER_AGENT] 表单已提交，注册成功；关键：两个 Agent 独立 ReAct 循环并行运行，互不阻塞
...
>
>
> 【图：图10-8 并行 Web Scraping 架构】
图中文字：Manager Agent；动态创建 · 实时监控 · 级联终止；Agent 1；cs.edu.cn；教师名录搜索；搜索中... ◎；Agent 2；math.edu.cn；未找到 ✗；Agent 3；phys.edu.cn；找到！ ✓；Agent 4；chem.edu.cn；已终止 ⊘；Agent 5 … 10；…（共 10 个）；级联终止时序；t=0s；启动 10 个 Agent；并行搜索"张伟"；t=12s；Agent 2 完成；未找到 → 退出；t=18s；Agent 3 找到!；发送 target_found；t=18.1s；Manager 广播 terminate；给其余运行中的 Agent；t=19s；全部确认终止；汇总结果返回；找到结果；姓名: 张伟 学院: 物理学院；职位: 教授 方向: 量子计算；邮箱: zhangwei@phys.edu.cn；性能对比；串行: 10 个网站 × 30s = ~5 分钟；并行: 18 秒找到 + 1 秒终止 = 19 秒；加速比: ~15×（含级联终止优化）
>
>
...
【图：图10-9 MetaGPT 多 Agent 协作网络】
图中文字：Product Manager；输入: 用户需求描述；产出:；功能列表 + 优先级；用户故事 (5 条)；验收标准；docs/PRD.md；Architect；输入: PRD.md；技术栈: FastAPI+React；API 规范 (OpenAPI)；数据库 Schema；docs/design.md；Project Manager；输入: design.md；任务清单 + 分工；文件级分配；模块依赖顺序；docs/tasks.md；Engineer ×3；输入: tasks.md + design.md；模块 A: 用户服务；模块 B: 订单服务；模块 C: 支付服务；src/*.py；QA Engineer；输入: src/ + PRD.md；单元测试 (pytest)；集成测试 (API)；Bug 报告 → Engineer；docs/test_report.md；Bug 修复；共享项目目录；docs/PRD.md docs/design.md docs/tasks.md src/*.py docs/test_report.md；MetaGPT 核心设计；▸ 标准化文档；每个角色输出明确格式 — 下游只需理解格式，不需理解上游思考过程；▸ 接口解耦；改进 Product Mgr（换更强模型）只要输出符合 PRD 格式，下游零修改；▸ 无 Manager；控制权沿 DAG 自然流动：Product Mgr→Architect→Project Mgr→Engineer→QA；▸ 异常通道；QA 测试失败 → Bug 报告按模块路由回 Engineer → 迭代修复
...
【图：图10-10 AI 小镇架构】
图中文字：Isabella Rodriguez；Hobbs 咖啡馆店主；热情好客，乐于社交；记忆流 Memory Stream；[08:30] Hobbs 咖啡馆开门营业；importance: 4 recency: 0.9；[09:15] 顾客 Klaus 来店里买咖啡；importance: 5 recency: 0.85；[10:00] 决定办一场情人节派对；importance: 9 recency: 0.8；[11:30] 邀请顾客 Maria 来派对；importance: 8 recency: 0.7；[14:00] 请 Maria 帮忙布置场地；importance: 7 recency: 0.6；反思 Reflection；"谁是 Hobbs 的常客？"；→ Maria、Klaus、Tom（常来）；"我该邀请谁来派对？"；→ 熟客与好友都邀请；"派对筹备到哪一步了？"；→ 已邀数人, 待布置场地；"谁能帮我布置咖啡馆？"；→ Maria（好友, 乐意帮忙）；计划与行动 Planning；08:00 起床 + 早餐；09:00 Hobbs 开店营业；12:00 边营业边邀请顾客；14:00 与 Maria 布置场地；16:00 备好茶点与座位；← 动态调整；18:00 在 Hobbs 举办情人节派对；检索；驱动；涌现行为（25 个 Agent · 2 天虚拟时间）；▸ 自发社交；多次偶遇 → 友谊形成 → 主动约见；▸ 信息传播；Isabella 的情人节派对邀请扩散到多个 Agent；▸ 选举传播；Sam 竞选市长的消息在 Agent 间扩散；▸ 关系记忆；记住过往交谈（如摄影项目）并延续话题；所有行为均非预编程 — 记忆 + 反思 + 社交常识推理的涌现结果
...
>
>
> 【图：图10-11 语音狼人杀 Agent 系统】
图中文字：法官（代码驱动）；游戏状态 · 阶段控制 · 信息分发；Night → Day → Vote → Settle；🐺 狼人 1；可见: 同伴身份；策略: 伪装村民；夜晚: 选择目标；互知；🐺 狼人 2；策略: 跟票保护；夜晚: 协商目标；🔮 预言家；可见: 验人结果；策略: 择机跳出；夜晚: 查验 1 人；验人结果；🧪 女巫；可见: 死亡/救治；策略: 保药/解药；夜晚: 救/毒 1 人；👤 村民 ×2；可见: 仅公开信息；策略: 逻辑推理；白天: 分析发言；信息权限控制：法官按角色过滤上下文；狼人:；所有狼人身份 + 夜晚商议 + 公开发言；预言家:；验人结果(仅自己验的) + 公开发言；女巫:；当晚死亡者 + 解药/毒药状态 + 公开发言；村民:；仅公开发言 + 投票记录（零私有信息）；实时语音交互（ASR + LLM + TTS）；白天讨论；法官管理发言顺序；按座位依次发言；投票阶段；收集所有玩家投票；统计票数公布结果；夜晚阶段；法官依次唤醒角色；私密语音通道；真人玩家；随机分配角色；语音表达投票/发言
>
>
